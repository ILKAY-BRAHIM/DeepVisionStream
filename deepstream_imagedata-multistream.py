...
# stream detections using socket
...
def start_ws_server():
    ...
    # MUST be called after setting the loop
...
def save_extracted_data_to_file():
    file_path = os.path.join(folder_name, "extracted_data.txt")
    ...
    while l_frame is not None:
        ...
        while l_obj is not None:
            try:
                ...
                break
            # print("Object id=", obj_meta.object_id, " Class id=", obj_meta.class_id)
            #! obj_counter[obj_meta.class_id] += 1
            # Periodically check for objects with borderline confidence value that may be false positive detections.
            # If such detections are found, annotate the frame with bboxes and confidence value.
            # Save the annotated frame to file.
            # if saved_count["stream_{}".format(frame_meta.pad_index)] % 10 == 0 and (
            ...
            if MIN_CONFIDENCE < obj_meta.confidence < MAX_CONFIDENCE:    
                if is_first_obj:
                    ...
                    n_frame = draw_bounding_boxes(n_frame, obj_meta, obj_meta.confidence, frame_number)
                    if n_frame is None:
...
def draw_bounding_boxes(image, obj_meta, confidence, frame_number):
    global EXTRACTED_DATA, ws_loop
    ...
    if obj_meta.class_id >= len(pgie_classes_str):
        return None
    ...
    image = cv2.putText(image, obj_name + ',C=' + str(confidence), (left - 10, top - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                        (0, 0, 255, 0), 2)
    # save x1,y1,x2,y2 coordinates of the bounding box
    x1 = left
    y1 = top
    x2 = left + width
    y2 = top + height
    ...
    # save the extracted data in a djictionary
    data = {
        "x1": x1,
        "y1": y1,
        "x2": x2,
        ...
        "frame_number": frame_number,
        "frame_base64": transform_image_to_base64(original_image)
    }
    ...
    print("Broadcasting data to connected clients")
